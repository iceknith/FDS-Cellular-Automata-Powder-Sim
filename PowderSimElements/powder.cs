...
public class Powder : Element
{
	...
	override public void update(Element[,] oldElementArray, Element[,] currentElementArray, int x, int y, int maxX, int maxY)
	{
		...
		if (x + 1 < maxX && 0 <= x - 1 && !Element.isMoreDense(x, y, x-1, y+1, oldElementArray) && !Element.isMoreDense(x, y, x+1, y+1, oldElementArray))
		{
		...
		// Diag Left movement
		if (0 <= x - 1 && !Element.isMoreDense(x, y, x-1, y+1, oldElementArray))
		{
			base.move(x, y, x - 1, y + 1, currentElementArray);
			return;
		}
		...
		// Diag Right movement
		if (x + 1 < maxX && !Element.isMoreDense(x, y, x+1, y+1, oldElementArray))
		{
			base.move(x, y, x + 1, y + 1, currentElementArray);
			return;
		}
